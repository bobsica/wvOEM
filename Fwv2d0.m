function [R,yf,J]  = Fwv2d0(Q, R, x, iter)
% Calculate Jacobians for forward model, 26 Jun 2015
...
[SHA,SNA,SH,SN] = forwardModelWV2d0(Q,x);

if ~isempty(find(isnan(x)) == 1)
    'after FM: Nans in retrieval vector (Fwv(n).m)'
    iter
    stop
end
...
% Jacobians for q
for j = 1:2*m %m
    [dSHdxA,dSNdxA,dSHdx,dSNdx] = derivSHSN2(Q,x,j,@forwardModelWV2d0);
...
% CN / CHA
[d1,d2,d3,d4] = derivBN2(Q,x,n-9,@forwardModelWV2d0);
Kernel(1:m1,n-9) = d1;
Kernel(m1+1:m2,n-9) = d2;
[d1,d2,d3,d4] = derivBN2(Q,x,n-8,@forwardModelWV2d0);
Kernel(1:m1,n-8) = d1;
Kernel(m1+1:m2,n-8) = d2;
[d1,d2,d3,d4] = derivBN2(Q,x,n-7,@forwardModelWV2d0);
Kernel(m2+1:m2+m3,n-7) = d3;
Kernel(m2+m3+1:mdata,n-7) = d4;

% angstrom exponent
[d1,d2,d3,d4] = derivBN2(Q,x,n-6,@forwardModelWV2d0);
Kernel(1:m1,n-6) = d1;
...
% dead times
[dSHdxA,dSNdxA,dSHdx,dSNdx] = derivSHSN2(Q,x,n-5,@forwardModelWV2d0);
Kernel(m2+1:m2+m3,n-5) = dSHdx;
[dSHdxA,dSNdxA,dSHdx,dSNdx] = derivSHSN2(Q,x,n-4,@forwardModelWV2d0);
Kernel(m2+m3+1:mdata,n-4) = dSNdx;

% backgrounds
[d1,d2,d3,d4] = derivBN2(Q,x,n-3,@forwardModelWV2d0);
Kernel(1:m1,n-3) = d1;
[d1,d2,d3,d4] = derivBN2(Q,x,n-2,@forwardModelWV2d0);
Kernel(m1+1:m2,n-2) = d2;
[d1,d2,d3,d4] = derivBN2(Q,x,n-1,@forwardModelWV2d0);
Kernel(m2+1:m2+m3,n-1) = d3;
[d1,d2,d3,d4] = derivBN2(Q,x,n,@forwardModelWV2d0);
